    ...
    def get_report(cls, report_summary: DataQualityReportSummary):
        ...
        try:
            ...
            for level in Level:
                ...
                warning_data[level.value]["count"] = len(df[df["importance"] == level.value])

            df = df.groupby(['observation','category','importance']).agg({
                                'vehicle_journey_id': 'size',
                }).rename(columns={'vehicle_journey_id':'number_of_services_affected',
                }).reset_index()

            for level in Level:
                warning_data[level.value] = {}
                warning_data[level.value]["count"] = df[df["importance"] == level.value]["number_of_services_affected"].sum()
                importance_df = df[df["importance"] == level.value]
                warning_data[level.value]["df"] = {}
                warning_data[level.value]["intro"] = CRITICAL_INTRO if level.value == "Critical" else ADVISORY_INTRO
                for category in importance_df["category"].unique():
                    warning_data[level.value]["df"].update({category: df[df["category"] == category]})
            return cls(data=warning_data, count=count, bus_services_affected=bus_services_affected)
