...
class ListView(PublishView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if section == "active":
            ...
            feeds_table = FaresDataFeedTable(active_feeds)
            ...
            draft_revisions = datasets.add_draft_revisions().add_draft_revision_data(
                organisation=self.organisation, dataset_types=[DatasetType.FARES]
            )
            feeds_table = FaresDataFeedTable(draft_revisions)
            RequestConfig(self.request, paginate={"per_page": self.per_page}).configure(
                feeds_table
            )
            ...
            feeds_table = FaresDataFeedTable(archive_feeds)
            RequestConfig(self.request, paginate={"per_page": self.per_page}).configure(
                feeds_table
            )
        ...
        context.update(
            {
                ...
                "page_title": f"{self.organisation.name} fares data sets",
            }
        )
        return context
