...
class PopulateLTA:
    ...
    def populate_lta(self, registrations_from_service):
        for registration_from_service in registrations_from_service:
            ...
            logger.info(
                f"Requesting OTC API for LTA name(s) for registration number - {registration_from_service[1]} and lta id is {lta_id}"
            )
            try:
            ...
            unique_local_authorities = set(
                reg.local_authorities for reg in registrations
            )
            unique_local_authorities = {
                "" if value is None else value for value in unique_local_authorities
            }
    ...
    def refresh(self, when: datetime):
        ...
        for registrations in self._client.get_latest_variations_since(when):
            service_id = Service.objects.filter(
                registration_number=registrations.registration_number
            ).values_list("id")
            logger.info(
                f"New registration that need update is: {registrations.registration_number}"
            )
