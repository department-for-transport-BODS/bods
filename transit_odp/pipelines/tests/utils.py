...
def get_base_csv(file_name_with_path: str) -> pd.DataFrame:
    ...
    base_csv = pd.read_csv(file_name_with_path, dtype={"vehicle_journey_code": str})
    ...
    base_csv["line_name"] = base_csv["line_name"].astype(str)
    base_csv["vehicle_journey_code"] = base_csv["vehicle_journey_code"].astype(str)
    return base_csv
