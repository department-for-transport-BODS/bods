...
def test_csv_output_order():
    ...
    assert columns[2] == "Registration Status"
    assert columns[3] == "Scope Status"
    assert columns[4] == "Seasonal Status"
    assert columns[5] == "Organisation Name"
    assert columns[6] == "Requires Attention"
    assert columns[7] == "Timetables requires attention"
    assert columns[8] == "Timetables Published Status"
    assert columns[9] == "Timetables Timeliness Status"
    assert columns[10] == "Timetables critical DQ issues"
    assert columns[11] == "AVL requires attention"
    assert columns[12] == "AVL Published Status"
    assert columns[13] == "Error in AVL to Timetable Matching"
    assert columns[14] == "Fares requires attention"
    assert columns[15] == "Fares Published Status"
    assert columns[16] == "Fares Timeliness Status"
    assert columns[17] == "Fares Compliance Status"
    assert columns[18] == "Timetables Data set ID"
    assert columns[19] == "TXC:Filename"
    assert columns[20] == "TXC: Revision number"
    assert columns[21] == "TXC: Derived termination date"
    assert columns[22] == "TXC:NOC"
    assert columns[23] == "TXC:Last Modified Date"
    assert columns[24] == "Date when timetable data is over 1 year old"
    assert columns[25] == "TXC:Operating Period End Date"
    assert columns[26] == "Fares Data set ID"
    assert columns[27] == "NETEX:Filename"
    assert columns[28] == "NETEX:Last Modified Date"
    assert columns[29] == "Date when fares data is over 1 year old"
    assert columns[30] == "NETEX:Operating Period End Date"
    assert columns[31] == "Date Registration variation needs to be published"
    assert columns[32] == "Date for complete 42 day look ahead"
    assert columns[33] == "Date seasonal service should be published"
    assert columns[34] == "Seasonal Start Date"
    assert columns[35] == "Seasonal End Date"
    assert columns[36] == "Registration:Operator Name"
    assert columns[37] == "Registration:Licence Number"
    assert columns[38] == "Registration:Service Type Description"
    assert columns[39] == "Registration:Variation Number"
    assert columns[40] == "Registration:Expiry Date"
    assert columns[41] == "Registration:Effective Date"
    assert columns[42] == "Registration:Received Date"
    assert columns[43] == "Traveline Region"
    assert columns[44] == "Local Transport Authority"
...
def test_service_in_bods_but_not_in_otc():
    ...
    for index, row in df.iterrows():
        assert row["Timetables Published Status"] == "Unpublished"
...
def test_service_in_bods_and_otc():
    ...
    df = _get_timetable_compliance_report_dataframe()
    for index, row in df.iterrows():
        dataset = Dataset.objects.get(id=row["Timetables Data set ID"])
        ...
        assert row["Timetables Published Status"] == "Published"
        assert row["Registration Status"] == "Registered"
        assert row["Scope Status"] == "In Scope"
        ...
        assert row["TXC:Filename"] == txc_file_attributes.filename
        assert row["TXC:NOC"] == txc_file_attributes.national_operator_code
        assert (
            row["TXC:Last Modified Date"]
            == (txc_file_attributes.modification_datetime).date()
        )
        assert (
            row["TXC:Operating Period End Date"]
...
def test_service_in_otc_and_not_in_bods():
    ...
    for _, row in df[1:].iterrows():
        ...
        assert row["Registration:Effective Date"] == service.effective_date
        assert row["Registration:Received Date"] == service.received_date
        assert row["Timetables requires attention"] == "Yes"
...
def test_stale_42_day_look_ahead(effective, modified, period_end, is_stale):
    ...
    df = _get_timetable_compliance_report_dataframe()
    assert (
        df["Timetables Timeliness Status"][0] == "42 day look ahead is incomplete"
    ) == is_stale
...
def test_stale_12_months_old(effective, modified, period_end, period_start, is_stale):
    ...
    assert df["Timetables requires attention"][0] == "Yes" if is_stale else "No"
    assert (
        df["Date when timetable data is over 1 year old"][0]
...
def test_not_stale():
    ...
    df = _get_timetable_compliance_report_dataframe()
    assert df["Timetables Timeliness Status"][0] == "Up to date"
    assert df["Timetables requires attention"][0] == "No"
...
def test_stale_service_out_of_season():
    ...
    df = _get_timetable_compliance_report_dataframe()
    assert df["Timetables Timeliness Status"][0] == "42 day look ahead is incomplete"
